...
{
  print <<EOF;
Best results if you provide allcallable matrices.
This script is not indel-safe, and will completely throw out any sample that contains an indel.

Usage:
combine_matrices.pl <matrix1.tsv> [matrix2.tsv ...] <output_matrix.tsv> <noNXindel_matrix.tsv> <allcallable_matrix.tsv> <output_matrix.snpfasta> <noNXindel_matrix.snpfasta>
EOF
  exit();
}

my @oldmatrixfiles = ();
while( my $oldmatrixfile = shift() ){ push( @oldmatrixfiles, $oldmatrixfile ); }
my $nonxsnpfastafile = pop( @oldmatrixfiles );
my $snpfastafile = pop( @oldmatrixfiles );
my $allcallmatrixfile = pop( @oldmatrixfiles );
my $nonxmatrixfile = pop( @oldmatrixfiles );
my $outputmatrixfile = pop( @oldmatrixfiles );

my $snppositions = {};
my $samplecallsdata = {};
my $referencecalls = {};

# Read in original SNP matrices
foreach my $oldmatrixfile (@oldmatrixfiles)
{
  if( open( my $oldmatrixhandle, '<', $oldmatrixfile ) )
  {
    ...
    if( ( $positioncolumn >= 0 ) && ( $referencecolumn >= 0 ) )
    {
      ...
      while( $linefromfile = <$oldmatrixhandle> )
      {
        ...
        if( defined( $referencecalls->{$currentchromosome}{$columnsfromfile[$positioncolumn]} ) && ( uc( $referencecalls->{$currentchromosome}{$columnsfromfile[$positioncolumn]} ) ne uc( $columnsfromfile[$referencecolumn] ) ) )
        {
          print STDERR "Are you crazy?! These matrices used different references! '$columnsfromfile[$referencecolumn]' is not '$referencecalls->{$currentchromosome}{$columnsfromfile[$positioncolumn]}' at position $columnsfromfile[$positioncolumn] in chromosome '$currentchromosome' from '$oldmatrixfile'\n";
        } else { $referencecalls->{$currentchromosome}{$columnsfromfile[$positioncolumn]} = $columnsfromfile[$referencecolumn]; }
        $i = 0;
        while( $i < scalar( @columnheadings ) )
        {
...
if( open( my $matrixfilehandle, '>', $outputmatrixfile ) && open( my $nonxfilehandle, '>', $nonxmatrixfile ) && open( my $allcallfilehandle, '>', $allcallmatrixfile ) )
{
  ...
  foreach my $samplefile ( sort keys( $samplecallsdata ) )
  {
    ...
    {
      print $matrixfilehandle "${samplefile}::$samplecolumn\t";
      print $nonxfilehandle "${samplefile}::$samplecolumn\t";
      print $allcallfilehandle "${samplefile}::$samplecolumn\t";
    }
  }
...
if( open( my $snpfastahandle, '>', $snpfastafile ) && open( my $nonxsnpfastahandle, '>', $nonxsnpfastafile ) )
{
  if( length( $refsnpfastadata ) )
  {
    ...
    foreach my $samplefile ( sort keys( $samplecallsdata ) )
    {
      foreach my $samplecolumn ( sort keys( $samplecallsdata->{$samplefile} ) )
      {
        print $snpfastahandle ">SNP::${samplefile}::$samplecolumn\n";
  ...
  if( length( $nonxrefsnpfastadata ) )
  {
    ...
    foreach my $samplefile ( sort keys( $samplecallsdata ) )
    {
      foreach my $samplecolumn ( sort keys( $samplecallsdata->{$samplefile} ) )
      {
        print $nonxsnpfastahandle ">SNP::${samplefile}::$samplecolumn\n";
