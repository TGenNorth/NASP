...
Position file should contain contig/chromosome name in the first column, and position number in the secold column, separated by a tab.
(VCF files are in this format.)

Usage:
remove_matrix_column.pl <original_matrix.tsv> <positionfile.txt> <output_matrix.tsv> <noNXindel_matrix.tsv> <allcallable_matrix.tsv> <output_matrix.snpfasta> <noNXindel_matrix.snpfasta>
EOF
  exit();
}

my $oldmatrixfile = shift();
my $positionfile = shift();
my $outputmatrixfile = shift();
my $nonxmatrixfile = shift();
my $allcallmatrixfile = shift();
my $snpfastafile = shift();
my $nonxsnpfastafile = shift();

if( open( my $positionhandle, '<', $positionfile ) )
{
  # Read in positions to exclude
  my $positionstoexclude = {};
  my $linefromfile = "";
  while( $linefromfile = <$positionhandle> )
  {
    if( $linefromfile =~ /^([^\t#][^\t]*)\t(\d{1,16})(?:\t|$)/ )
    {
      my $chromosomename = $1;
      my $positionnumber = $2;
      if( !defined( $positionstoexclude->{$chromosomename} ) ){ $positionstoexclude->{$chromosomename} = {}; }
      $positionstoexclude->{$chromosomename}{$positionnumber} = 1;
    }
  }
  close( $positionhandle );
      ...
      if( open( my $matrixfilehandle, '>', $outputmatrixfile ) && open( my $nonxfilehandle, '>', $nonxmatrixfile ) && open( my $allcallfilehandle, '>', $allcallmatrixfile ) )
      {
        ...
        foreach my $currentchromosome ( sort keys( %$snppositions ) )
        {
          foreach my $currentposition ( sort {$a <=> $b} keys( $snppositions->{$currentchromosome} ) )
          {
            ...
            if( ( $allelecounts->{'snp'} > 0 ) && !defined( $positionstoexclude->{$currentchromosome}{$currentposition} ) )
            {
...
} else { print STDERR "Could not open '$positionfile'!\n"; }
