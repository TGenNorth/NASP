...
sub nasp
{
  ...
  if( $runbwa || $runbwamem || $runnovo || $findbams )
  {
    ...
    while( $userinput !~ /^$|^[YNyn]/ )
    {
      print "Would you like to run SAMtools [N]?* ";
      $userinput = <>;
      if( $userinput =~ /^[Yy]/ ){ $runsamtools = 1; }
    }
  }
  $userinput = 'X';
  while( $userinput !~ /^$|^[YNyn]/ )
  ...
  if( opendir( my $readfolderhandle, $readfilefolder ) )
  {
    ...
    if( ( scalar( @fastafilelist ) + scalar( @fastqfilelist ) + scalar( @bamfilelist ) + scalar( @vcffilelist ) ) >= 1 )
    {
      ...
      print $loghandle "  Check for duplicates: " . ( $finddupregions ? "Yes" : "No" ) . "\n";
      print $loghandle "  External genomes: ";
      if( $findexternalfastas ){ print $loghandle "\n    " . join( "\n    ", @fastafilelist ) . "\n"; } else { print $loghandle "none\n"; }
      print "Fasta data:\n  Reference file: $referencefastafile\n";
      print "  Check for duplicates: " . ( $finddupregions ? "Yes" : "No" ) . "\n";
      ...
      if( $pipelinestartqid =~ /^\d{1,31}\.\w/ )
      {
        ...
        if( scalar( @finalfilelist ) > 0 )
        {
          ...
          {
            my $commandtorun = "$matrixmakingscript $mincoverage $minproportion $referencefastafile $finalfilestring $outputfilefolder/snp_matrix.tsv $outputfilefolder/snp_matrix_noNXindel.tsv $outputfilefolder/snp_matrix_intersect.tsv $outputfilefolder/snp_matrix_allcallable.tsv $outputfilefolder/snp_matrix.snpfasta $outputfilefolder/snp_matrix_noNXindel.snpfasta $outputfilefolder/snp_matrix_intersect.snpfasta $outputfilefolder/filter_log.txt $outputfilefolder/statistics.tsv \n";
...
sub _submit_samtools
{
  ...
  my $bamfilenickname = shift();
  my $loghandle = shift();
  my $commandtorun = "$samtoolspath mpileup -uDf $referencefastafile $inputfilefolder/$bamfilename | $bcftoolspath view -ceg - > $outputfilefolder/samtools/$bamfilenickname-samtools.vcf \n";
