...
def _index_reference( configuration ):
    ...
    if next((v for i,v in enumerate(configuration["snpcallers"]) if re.search('gatk', v[0], re.IGNORECASE)), None):
        ...
        index_commands.append("java -Xmx%sG -jar %s R=%s O=%s.dict" % (picard_memory, dict_generator, reference, reference))
...
def _run_bwa(read_tuple, aligner, samtools, job_submitter, index_job_id, reference, output_folder):
    ...
    if re.search('mem', aligner_name, re.IGNORECASE):
        ...
        output_folder = os.path.join(output_folder, aligner_name)
        command_parts = []
        if read2:
            output_file_1 = os.path.join(output_folder, "%s-R1.sai" % name)
            output_file_2 = os.path.join(output_folder, "%s-R2.sai" % name)
            ...
            output_file = os.path.join(output_folder, "%s.sai" % name)
    ...
    command = "%s | %s | %s \n %s \n" % (aligner_command, samview_command, samsort_command, samindex_command)
...
def _run_gatk(nickname, bam_file, snpcaller, job_submitter, aligner_job_id, reference, output_folder):
    ...
    snpcaller_name = "gatk"
    ncpus = job_parms['num_cpus']
    memory = job_parms['memory_requested']
...
def _run_varscan(nickname, bam_file, snpcaller, samtools, job_submitter, aligner_job_id, reference, output_folder):
    ...
    memory = job_parms['memory_requested']
    vcf_nickname = "%s-%s" % (nickname, snpcaller_name)
    read_nickname = re.match('^(.*)-[a-z]*$', nickname, re.IGNORECASE).group(0)
    work_dir = os.path.join(output_folder, snpcaller_name)
...
def _find_dups( configuration, index_job_id, reference ):
    ...
    command = "find_duplicates.py %s %s" % (path, reference)
...
def _convert_external_genome( assembly, configuration, index_job_id, reference ):
    ...
    (name, file) = assembly
    command = "convert_external_genome.py --nucmerpath %s --nucmerargs %s --deltafilterpath %s --reference %s --external %s --name %s" % (nucmer_path, args, path, reference, file, name)
...
def main():
    ...
    (bam_files, bamindex_job_id) = _index_bams(configuration, index_job_id) if configuration["alignments"] else None
    for (name, file) in bam_files:
        snpcaller_output = _call_snps( (name, bamindex_job_id, file, "pre-aligned"), configuration, reference )
